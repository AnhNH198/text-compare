...
class buy(Resource):
    def post(self):
        posted_data = request.get_json()
        username = posted_data["username"]
        passwd = posted_data["admin_passwd"]
        add_amount = posted_data["token"]
        ...
        correct_admin_passwd = "1234566"
        if not passwd == correct_admin_passwd:
            ret_json = {
                "Message": "Invalid password",
                "Status code": 302
            }
        ...
        current_token = count_token(username)
        users.update(
            {"username": username},
            {"$set":
                {"token": current_token + add_amount}
             }
                    )
        ret_json = {
